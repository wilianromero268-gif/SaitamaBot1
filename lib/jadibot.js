...
import { translateText } from './lang.js'
...
export async function startSubBot(mainBot, numero, m = null) {
  ...
  const _sendMessage = subBot.sendMessage.bind(subBot)

subBot.sendMessage = async (jid, content = {}, options = {}) => {
  try {
    if (content?.text) {
      content.text = await translateText(subBot, content.text)
    }

    if (content?.caption) {
      content.caption = await translateText(subBot, content.caption)
    }

    if (content?.footer) {
      content.footer = await translateText(subBot, content.footer)
    }

    if (content?.title) {
      content.title = await translateText(subBot, content.title)
    }
  } catch (e) {
    console.error('[LANG SUBBOT]', e)
  }
  ...
  if (meta[numero].language) subBot.language = meta[numero].language
  }
